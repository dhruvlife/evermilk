...
class SubscriptionCubit extends Cubit<SubscriptionState> {
  ...
  Future<void> subscriptionAdd({required SubScribeCartParam subScribeCartParam}) async {
    ...
    if (schedule == AppStrings.dayWise && dayWiseQuantities.every((q) => q == 0)) {
    ...
    result.fold(
      ...
      (response) {
        if (response.message == AppStrings.subscriptionSuccess) {
          emit(SubscriptionSuccess(response.message ?? ''));
